 ...
 * @param {string} siteUrl - Site URL (default: from V4V_SITE_URL env or shawnyeager.com)
...
export async function fetchEssayTitles(siteUrl = null, forceRefresh = false) {
  const site = siteUrl || process.env.V4V_SITE_URL || 'shawnyeager.com';
  ...
  // Fetch RSS feed
  const feedUrl = `https://${site.replace(/^https?:\/\//, '')}/feed.xml`;
